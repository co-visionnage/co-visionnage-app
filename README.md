...
Интеграционные тесты (`integration/`) — тоже нужен настоящий Postgres
(миграции применены), но без `next dev`: тесты бьют напрямую в БД через `pg`,
тем же `withUserContext`-паттерном, что и приложение, чтобы проверять
реальные RLS-политики и SQL-функции (не моки). Фикстуры (профили, семьи)
сидируются через `MIGRATE_DATABASE_URL` (суперпользователь) — как и
`pnpm migrate`, потому что настоящая регистрация идёт через SECURITY DEFINER
функции, а не голый INSERT, который RLS отклонит от `app_user`. Настройте
`.env` так же, как для e2e (нужны обе переменные), и:
